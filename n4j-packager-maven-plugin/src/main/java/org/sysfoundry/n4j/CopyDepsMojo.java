...
@Mojo( name = "copy-deps", requiresDependencyResolution = ResolutionScope.TEST, defaultPhase = LifecyclePhase.PROCESS_SOURCES, threadSafe = true )
//CHECKSTYLE_ON: LineLength
public class CopyDepsMojo
    extends AbstractFromDependenciesMojo
{
        ...
        throws MojoExecutionException
    {
        ...
        Artifact appLauncherArtifact = null;

        if ( !useRepositoryLayout )
        {
            for ( Artifact artifact : artifacts )
            {
                if(!artifact.getArtifactId().equalsIgnoreCase("n4j-app-launcher")) {
                    copyArtifact(artifact, isStripVersion(), this.prependGroupId, this.useBaseVersion,
                            this.stripClassifier);
                }else{
                    appLauncherArtifact = artifact;
                }
            }
        }
        else
        {
            File libsOutputDirectory = new File(outputDirectory,"libs");
        ...
        //unpack the app-launcher classes
        ArchiverManager archiverManager = getArchiverManager();


        try {
            UnArchiver jarUnArchiver = archiverManager.getUnArchiver("jar");
            ArtifactHandler jarArtifactHandler = artifactHandlerManager.getArtifactHandler("jar");
            /*DefaultArtifact appLauncherArtifact  = new DefaultArtifact("org.sysfoundry.n4j","n4j-app-launcher",
                    "1.0-SNAPSHOT","compile","jar","",jarArtifactHandler);*/

            unpack( appLauncherArtifact, outputDirectory, "**/*.class", "", "UTF-8", jarUnArchiver.getFileMappers());
        ...
        throws MojoExecutionException
    {
        ...
        destDir = DependencyUtil.getFormattedOutputDirectory( useSubDirectoryPerScope, useSubDirectoryPerType,
                                                              ...
                                                              stripVersion, libsOutputDirectory, artifact );
